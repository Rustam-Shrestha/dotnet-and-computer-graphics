...
public class BresenhamsLineDrawing extends Frame { 
    ...
    private void bresenhamLine(Graphics g, int x1, int y1, int x2, int y2) { 
        ...
        while (true) { 
            ...
            if (counter % 6 == 0) { 
                g.drawLine(prevX, prevY, x1, y1); // Draw a line from the last point to the current point
                prevX = x1; // Update previous point
                prevY = y1;
            } 
        ...
        // Draw the final line segment to ensure the last point is connected
