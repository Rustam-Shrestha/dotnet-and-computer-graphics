...
public class SquareRotation3D extends JPanel {
    private double angle; // Rotation angle in degrees
    private int[][] square1, square2; // Coordinates for the squares

    public SquareRotation3D(double angle) {
        this.angle = Math.toRadians(angle); // Convert to radians for rotation calculations
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Apply rotation to squares
        int[][] rotatedSquare1 = rotate(square1, angle);
        int[][] rotatedSquare2 = rotate(square2, angle);

        // Draw rotated squares and connections
        drawSquare(g2d, rotatedSquare1, Color.BLUE, "Rotated");
        drawSquare(g2d, rotatedSquare2, Color.BLUE, "Rotated");

        // Draw lines connecting corresponding edges after rotation
    ...
    private int[][] rotate(int[][] square, double angle) {
        int[][] rotatedSquare = new int[square.length][2];
        ...
        for (int i = 0; i < square.length; i++) {
            ...
            // Apply rotation using homogeneous matrix
            rotatedSquare[i][0] = (int) (x * Math.cos(angle) - y * Math.sin(angle)) + cx;
            rotatedSquare[i][1] = (int) (x * Math.sin(angle) + y * Math.cos(angle)) + cy;
        }
    ...
    public static void main(String[] args) {
        ...
        JFrame frame = new JFrame("3D Square Rotation");
        SquareRotation3D panel = new SquareRotation3D(angle);
        frame.add(panel);
