...
public class ZXPlaneRotation extends JPanel {
    ...
    public ZXPlaneRotation(double theta) {
        ...
        // Perform rotation around the ZX plane
        rotatedSquare1 = rotateZX(originalSquare1);
        rotatedSquare2 = rotateZX(originalSquare2);
    }
    ...
    private int[][] rotateZX(int[][] square) {
        int[][] rotatedSquare = new int[square.length][3];
        for (int i = 0; i < square.length; i++) {
            rotatedSquare[i][0] = (int) (square[i][0] * Math.cos(theta) - square[i][2] * Math.sin(theta)); // X rotates
            rotatedSquare[i][1] = square[i][1]; // Y remains the same
            rotatedSquare[i][2] = (int) (square[i][0] * Math.sin(theta) + square[i][2] * Math.cos(theta)); // Z rotates
        }
    ...
    public static void main(String[] args) {
        JFrame frame = new JFrame("Rotation around ZX Plane");
        // take integer input from java
        Scanner sc = new Scanner(System.in);
        System.out.println("Enter the number of degrees: ");
        int theta = sc.nextInt();
        ZXPlaneRotation panel = new ZXPlaneRotation(Math.toRadians(theta)); // Example rotation angle
        frame.add(panel);
