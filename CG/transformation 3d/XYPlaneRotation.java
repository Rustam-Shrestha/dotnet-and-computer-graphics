...
public class XYPlaneRotation extends JPanel {
    ...
    public XYPlaneRotation(double theta) {
        ...
        // Perform rotation around the XY plane
        rotatedSquare1 = rotateXY(originalSquare1);
        rotatedSquare2 = rotateXY(originalSquare2);
    }
    ...
    private int[][] rotateXY(int[][] square) {
        int[][] rotatedSquare = new int[square.length][3];
        for (int i = 0; i < square.length; i++) {
            rotatedSquare[i][0] = (int) (square[i][0] * Math.cos(theta) - square[i][1] * Math.sin(theta)); // X rotates
            rotatedSquare[i][1] = (int) (square[i][0] * Math.sin(theta) + square[i][1] * Math.cos(theta)); // Y rotates
            rotatedSquare[i][2] = square[i][2]; // Z remains the same
        }
    ...
    public static void main(String[] args) {
        JFrame frame = new JFrame("Rotation around XY Plane");
        // take integer input from java
        Scanner sc = new Scanner(System.in);
        System.out.println("Enter the number of degree: ");
        int theta = sc.nextInt();
        XYPlaneRotation panel = new XYPlaneRotation(Math.toRadians(theta)); // Example rotation angle
        frame.add(panel);
