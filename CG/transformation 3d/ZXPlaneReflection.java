...
public class ZXPlaneReflection extends JPanel {
    ...
    public ZXPlaneReflection() {
        ...
        // Perform reflection across the ZX plane (Y flips sign)
        reflectedSquare1 = reflectZX(originalSquare1);
        reflectedSquare2 = reflectZX(originalSquare2);
    }
    ...
    private int[][] reflectZX(int[][] square) {
        int[][] reflectedSquare = new int[square.length][3];
        for (int i = 0; i < square.length; i++) {
            reflectedSquare[i][0] = square[i][0]; // X remains the same
            reflectedSquare[i][1] = -(square[i][1]); // Y changes sign (reflection across ZX plane)
    ...
    public static void main(String[] args) {
        JFrame frame = new JFrame("Reflection across ZX Plane");
        ZXPlaneReflection panel = new ZXPlaneReflection();
        frame.add(panel);
