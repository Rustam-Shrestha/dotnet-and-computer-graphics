...
public class ReflectionYEqualsMXPlusC extends Frame {
    private int[][] points;
    private int[][] reflectedPoints;
    private double m, c; // Slope and intercept

    public ReflectionYEqualsMXPlusC() {
        ...
        // Apply all transformations: T_back * R_back * Reflect * R_to_xaxis * T_to_origin * points
        for (int i = 0; i < points.length; i++) {
            int[] transformedPoint = points[i];
            transformedPoint = matrixMultiplication(transformedPoint, translateToOrigin);
            transformedPoint = matrixMultiplication(transformedPoint, rotateToXAxis);
            transformedPoint = matrixMultiplication(transformedPoint, reflectAcrossXAxis);
            transformedPoint = matrixMultiplication(transformedPoint, rotateBack);
            transformedPoint = matrixMultiplication(transformedPoint, translateBack);
