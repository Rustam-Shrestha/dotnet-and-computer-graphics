...
public class HomogeneousTranslationX extends Frame {
    private int[][] points;
    private int[][] translatedPoints;
    private int tx;

    public HomogeneousTranslationX() {
        ...
        // Get the translation factor for X direction
        System.out.println("Enter translation factor tx:");
        tx = s.nextInt();
        
        // Apply translation to the points (only in X direction)
        translatedPoints = new int[points.length][3];
        for (int i = 0; i < points.length; i++) {
            translatedPoints[i][0] = points[i][0] + tx;
            translatedPoints[i][1] = points[i][1]; // Y remains the same
            translatedPoints[i][2] = 1; // Homogeneous coordinate
        }

        // Print the translated points
        System.out.println("Translated Points (in X direction):");
        for (int i = 0; i < numPoints; i++) {
            System.out.println("(" + translatedPoints[i][0] + ", " + translatedPoints[i][1] + ")");
        }

        this.setTitle("Homogeneous Translation in X-Direction");
