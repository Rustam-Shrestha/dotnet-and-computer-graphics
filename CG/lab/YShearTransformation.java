...
public class YShearTransformation extends Frame {
    ...
    public YShearTransformation() {
        ...
        // Get the shear factor for y-direction
        ...
        // Shearing matrix for y-direction
        double[][] shearMatrixY = {
            {1, 0, 0},
            {shy, 1, 0},
            {0, 0, 1}
        ...
        // Perform the transformation T(refPoint) * S(shearY) * T(-refPoint)
        for (int i = 0; i < points.length; i++) {
            int[] translatedToRef = matrixMultiplication(points[i], toReference);
            double[] sheared = matrixMultiplicationDouble(translatedToRef, shearMatrixY);
            shearedPoints[i] = matrixMultiplication(sheared, backToOriginal);
        }

        this.setTitle("Shear Transformation in Y-direction with Reference Point Translation");
