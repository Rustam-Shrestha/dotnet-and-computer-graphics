package done.finalyzer;

import java.awt.*;
import java.util.Scanner;

public class CounterClockWiseRotation extends Frame {
    private int[][] points;
    private int[][] rotatedPoints;
    private double angle; // Rotation angle
    private int px, py; // Midpoint (centroid)

    public CounterClockWiseRotation() {
        ...
        angle = Math.toRadians(s.nextDouble()); // Convert degrees to radians

        // Apply rotation
        rotatedPoints = new int[points.length][3];
        ...
        // Combine the matrices: T(mid) * R * T(-mid)
        double[][] finalMatrix = new double[3][3];
        double[][] tempMatrix = new double[3][3];

        // First multiply rotation matrix with negative translation
        multiplyMatrices(translationToMidpoint, rotationMatrix, tempMatrix);
        ...
        for (int i = 0; i < points.length; i++) {
            rotatedPoints[i] = matrixMultiplication(finalMatrix, points[i]);
        }

        this.setTitle("CCW Rotation");
    ...
    public void paint(Graphics g) {
        ...
        drawShape(g, rotatedPoints, "Rotated");
    }
